...
// --- Recupero collaboratori e personaggi frequenti ---
...
foreach ($author_roles as $role_key) {
    ...
    // Ordinamento cronologico (dal più vecchio al più nuovo) come default
    $order_by_comic_sql = "ORDER BY c.publication_date ASC, CAST(REPLACE(REPLACE(c.issue_number, ' bis', '.5'), ' ter', '.7') AS DECIMAL(10,2)) ASC";
    ...
    // Ordinamento cronologico (dal più vecchio al più nuovo) come default
    $order_by_story_sql = "ORDER BY c.publication_date ASC, CAST(REPLACE(REPLACE(c.issue_number, ' bis', '.5'), ' ter', '.7') AS DECIMAL(10,2)) ASC, s.sequence_in_comic ASC";
...
$sql_latest_stories = "
    ...
    ORDER BY c.publication_date ASC, CAST(REPLACE(REPLACE(c.issue_number, ' bis', '.5'), ' ter', '.7') AS DECIMAL(10,2)) ASC
    LIMIT 50 
";
        ...
        <div class="author-info">
            ...
            <?php if (!empty($author['biography'])): ?>
                <div class="biography-container">
                    <p class="biography collapsed" id="biography-text"><?php echo nl2br(htmlspecialchars($author['biography'])); ?></p>
                    <button class="biography-toggle" id="biography-toggle" onclick="toggleBiography()">Mostra tutto</button>
                </div>
            <?php endif; ?>
        </div>
    </div>
        ...
        <div class="contributions-section">
            ...
            <div class="author-roles-tab-content-panels">
                <div id="latest-inserted" class="author-role-tab-content active">
                    <div class="tab-content-header">
                        <div class="sort-controls">
                            <label for="sort-latest-inserted">Ordina per:</label>
                            <select id="sort-latest-inserted" onchange="sortTabContent('latest-inserted')">
                                <option value="chronological">Cronologico (pubblicazione)</option>
                                <option value="reverse-chronological">Cronologico inverso (pubblicazione)</option>
                                <option value="newest">Più recenti prima (inserimento)</option>
                                <option value="oldest">Più vecchie prima (inserimento)</option>
                            </select>
                        </div>
                    </div>
                    <div class="tab-items-container">
                        <?php if (!empty($latest_inserted_stories)): ?>
                            <?php foreach ($latest_inserted_stories as $contrib): ?>
                                <div class="contribution-item" data-publication-date="<?php echo $contrib['publication_date']; ?>" data-created-date="<?php echo $contrib['story_created_at']; ?>">
                                    <div class="contribution-cover">
                                        ...
                                        <?php else: ?>
                                            <?php echo generate_image_placeholder(htmlspecialchars($contrib['issue_number']), 70, 100, 'comic-list-placeholder'); ?>
                                        <?php endif; ?>
                                        </a>
                                    </div>
                                    <div class="contribution-details">
                                        <h4>
                                            <a href="<?php echo $link_href_latest; ?>" title="Vai al dettaglio dell'albo">
                                                Topolino #<?php echo htmlspecialchars($contrib['issue_number']); ?>
                                                <?php if(!empty($contrib['comic_title'])): ?> - <?php echo htmlspecialchars($contrib['comic_title']); ?><?php endif; ?>
                                            </a>
                                        </h4>
                                        <p class="story-title-link"><strong>Storia:</strong> <a href="<?php echo $link_href_latest; ?>#story-item-<?php echo $contrib['story_id']; ?>" title="Vai direttamente alla storia nell'albo">
                                            <?php
                                            ...
                                            if (!empty($contrib['story_title_main'])) {
                                                ...
                                                if (!empty($contrib['part_number'])) {
                                                    ...
                                                    if ($part_specific_title_latest !== htmlspecialchars($contrib['story_title_main']) && $part_specific_title_latest !== $expected_part_title_latest && strtolower($part_specific_title_latest) !== strtolower($expected_part_title_latest) ) {
                                                        ...
                                                        $display_story_title_latest .= ' - Parte ' . htmlspecialchars($contrib['part_number']); 
                                                    }
                                                ...
                                                if ($contrib['total_parts']) { 
                                                    $display_story_title_latest .= ' (di ' . htmlspecialchars($contrib['total_parts']) . ')'; 
                                                }
                                            } else { 
                                                $display_story_title_latest = htmlspecialchars($contrib['story_title']); 
                                            }
                ...
                <div id="<?php echo $role_id_variant; ?>" class="author-role-tab-content">
                    <div class="tab-content-header">
                        <div class="sort-controls">
                            <label for="sort-<?php echo $role_id_variant; ?>">Ordina per:</label>
                            <select id="sort-<?php echo $role_id_variant; ?>" onchange="sortTabContent('<?php echo $role_id_variant; ?>')">
                                <option value="chronological">Cronologico (dal più vecchio)</option>
                                <option value="reverse-chronological">Cronologico inverso (dal più recente)</option>
                            </select>
                        </div>
                    </div>
                    <div class="tab-items-container">
                        <?php foreach ($variant_covers_by_person as $contrib): ?>
                            <div class="contribution-item" data-publication-date="<?php echo $contrib['publication_date']; ?>">
                                <div class="contribution-cover">
                                ...
                                <div class="contribution-details">
                                   <h4><a href="<?php echo $link_href; ?>">Topolino #<?php echo htmlspecialchars($contrib['issue_number']); ?><?php if(!empty($contrib['comic_title'])): ?> - <?php echo htmlspecialchars($contrib['comic_title']); ?><?php endif; ?></a></h4>
               ...
               <?php foreach ($contributions_by_role as $role_name => $contributions_list): 
                   ...
                   <div id="<?php echo $panel_id; ?>" class="author-role-tab-content">
                       <div class="tab-content-header">
                           <div class="sort-controls">
                               <label for="sort-<?php echo $panel_id; ?>">Ordina per:</label>
                               <select id="sort-<?php echo $panel_id; ?>" onchange="sortTabContent('<?php echo $panel_id; ?>')">
                                   <option value="chronological">Cronologico (dal più vecchio)</option>
                                   <option value="reverse-chronological">Cronologico inverso (dal più recente)</option>
                               </select>
                           </div>
                           ...
                           <?php if (!empty($contributions_list)): ?>
                               <?php foreach ($contributions_list as $contrib): ?>
                                   <div class="contribution-item" data-publication-date="<?php echo $contrib['publication_date']; ?>">
                                       ...
                                       <div class="contribution-details">
                                           <h4><a href="<?php echo $link_href; ?><?php if ($contrib['type'] === 'story_role' && $contrib['story_id']) echo '#story-item-' . $contrib['story_id']; ?>">Topolino #<?php echo htmlspecialchars($contrib['issue_number']); ?><?php if(!empty($contrib['comic_title'])): ?> - <?php echo htmlspecialchars($contrib['comic_title']); ?><?php endif; ?></a></h4>
                                                   ...
                                                   if (!empty($contrib['story_title_main'])) {
                                                       ...
                                                       if (!empty($contrib['part_number'])) {
                                                           ...
                                                           if ($part_specific_title_author !== htmlspecialchars($contrib['story_title_main']) && $part_specific_title_author !== $expected_part_title_author && strtolower($part_specific_title_author) !== strtolower($expected_part_title_author) ) {
                                                               ...
                                                               $display_story_title_author .= ' - Parte ' . htmlspecialchars($contrib['part_number']); 
                                                           }
                                                       } elseif (!empty($contrib['story_title']) && strtolower($contrib['story_title']) !== strtolower($contrib['story_title_main'])) {
                                                           $display_story_title_author .= ': ' . htmlspecialchars($contrib['story_title']);
                                                       }
                                                       if ($contrib['total_parts']) { 
                                                           $display_story_title_author .= ' (di ' . htmlspecialchars($contrib['total_parts']) . ')'; 
                                                       }
                                                   } else { 
                                                       $display_story_title_author = htmlspecialchars($contrib['story_title']); 
                                                   }
...
// Funzioni per la biografia e l'ordinamento
function toggleBiography() {
   const biographyText = document.getElementById('biography-text');
   const toggleButton = document.getElementById('biography-toggle');
   
   if (biographyText.classList.contains('collapsed')) {
       biographyText.classList.remove('collapsed');
       toggleButton.textContent = 'Mostra meno';
   } else {
       biographyText.classList.add('collapsed');
       toggleButton.textContent = 'Mostra tutto';
   }
}

function sortTabContent(tabId) {
   const tab = document.getElementById(tabId);
   if (!tab) return;
   
   const container = tab.querySelector('.tab-items-container');
   if (!container) return;
   
   const sortSelect = document.getElementById('sort-' + tabId);
   ...
   items.sort((a, b) => {
       ...
       switch (sortValue) {
           case 'newest':
               dateA = new Date(a.getAttribute('data-created-date') || '1900-01-01');
               dateB = new Date(b.getAttribute('data-created-date') || '1900-01-01');
           ...
           case 'chronological':
               dateA = new Date(a.getAttribute('data-publication-date') || '1900-01-01');
               dateB = new Date(b.getAttribute('data-publication-date') || '1900-01-01');
   ...
   // Rimuovi tutti gli elementi e riaggiungi nell'ordine corretto
   items.forEach(item => container.removeChild(item));
   items.forEach(item => container.appendChild(item));
   
   // Riapplica la paginazione se necessaria
   const paginationContainer = tab.querySelector('.client-pagination');
   if (paginationContainer) {
       const firstPageBtn = paginationContainer.querySelector('.client-pagination-btn');
       if (firstPageBtn) {
           firstPageBtn.click();
       }
   }
