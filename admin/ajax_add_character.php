...
if (isset($_FILES['character_image']) && $_FILES['character_image']['error'] == UPLOAD_ERR_OK) {
    ...
    $original_filename = $_FILES['character_image']['name'];
    ...
    if (move_uploaded_file($_FILES['character_image']['tmp_name'], $temp_uploaded_file_absolute)) {
        $jpeg_quality = 75;
        $png_compression = 9;
        $convertToWebp = true;
        ...
        if ($processed_image_absolute_path) {
            ...
            error_log("Compressione fallita per character_image (Quick Add), usando " . $image_path_final);
        }
    } else {
...
try {
    ...
    if ($is_contributor && !$is_true_admin) {
        ...
        $stmt_pending = $mysqli->prepare("INSERT INTO pending_characters
            ...
            VALUES (NULL, ?, 'add', ?, ?, ?, NULL, NULL, NULL, 0)");
        $stmt_pending->bind_param("isss", $current_user_id_for_proposal, $name, $description, $image_path_final);
        ...
        echo json_encode([
            'success' => true, 
            'message' => 'Proposta personaggio inviata per revisione',
            'is_proposal' => true
        ]);
        ...
        $stmt_check = $mysqli->prepare("SELECT character_id FROM characters WHERE name = ?");
        ...
        if ($result_check->num_rows > 0) {
            $stmt_check->close();
            throw new Exception("Esiste già un personaggio con questo nome");
        }
        $stmt_check->close();

        // Genera slug
        $exclude_id_for_slug = 0;
        $slug = generate_slug($name, $mysqli, 'characters', 'slug', $exclude_id_for_slug, 'character_id');

        $stmt = $mysqli->prepare("INSERT INTO characters (name, slug, description, character_image, first_appearance_comic_id, first_appearance_story_id, first_appearance_date, first_appearance_notes, is_first_appearance_verified, created_at) VALUES (?, ?, ?, ?, NULL, NULL, NULL, NULL, 0, NOW())");
        $stmt->bind_param("ssss", $name, $slug, $description, $image_path_final);

        if (!$stmt->execute()) {
            if ($mysqli->errno === 1062) {
                throw new Exception("Esiste già un personaggio con questo nome o slug simile");
            } else {
                throw new Exception("Errore durante l'aggiunta: " . $stmt->error);
            }
        }

        $new_character_id = $mysqli->insert_id;
        $stmt->close();

        $mysqli->commit();
        echo json_encode([
            'success' => true, 
            'message' => 'Personaggio aggiunto con successo',
